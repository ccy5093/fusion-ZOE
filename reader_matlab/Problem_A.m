...
% Model error
Q_f = zeros(5);
Q_f(1,1) = 1;Q_f(2,2) = 1;Q_f(3,3) = 1;
Q_f(4,4) = 1*speed_std_f^2;Q_f(5,5) = 1*yaw_rate_std_f^2;
Q_f = Q_f*0.1;
Q_l = zeros(5);
...
% Observation error
R_f = eye(5);
R_f(1,1) = 1*x_std_f^2; R_f(2,2) = 1*y_std_f^2; R_f(3,3) = 1*yaw_std_f^2;
R_f(4,4) = 1*speed_std_f^2; R_f(5,5) = 1*yaw_rate_std_f^2;
R_f(3,3) = R_f(3,3)*1;R_f = R_f*3;
R_l = eye(5);
...
disp(['Accuracy(MSE) of the estimate on the follower : x-', num2str(Acc_f_x),' | y-', num2str(Acc_f_y),' | yaw-', num2str(Acc_f_yaw)])
disp(['Consistency of the estimate on the follower: ', num2str(100*consis_f),'%'])
disp(['Accuracy(MSE) of the estimate on the leader : x-', num2str(Acc_l_x),' | y-', num2str(Acc_l_y),' | yaw-', num2str(Acc_l_yaw)])
...
subplot(3,2,2)
plot(result_f.timestamp,result_f.err(1,:))
hold on; grid on
...
subplot(3,2,4)
plot(result_f.timestamp,result_f.err(2,:))
hold on; grid on
...
subplot(3,2,6)
plot(result_f.timestamp,result_f.err(3,:))
hold on; grid on
