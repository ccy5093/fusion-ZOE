...
% Model error
Q_f = zeros(5);
Q_f(1,1) = 1;Q_f(2,2) = 1/3;Q_f(3,3) = 1;
Q_f(4,4) = speed_std_f^2;Q_f(5,5) = yaw_rate_std_f^2;
Q_f = Q_f*0.4;
Q_l = zeros(5);
Q_l(1,1) = 1;Q_l(2,2) = 1;Q_l(3,3) = 0.1;
Q_l(4,4) = speed_std_l^2;Q_l(5,5) = yaw_rate_std_l^2;
Q_l = Q_l*0.1;
% Observation error
R_f = eye(5);
R_f(1,1) = x_std_f^2; R_f(2,2) = 1*y_std_f^2; R_f(3,3) = 1*yaw_std_f^2;
R_f(4,4) = speed_std_f^2; R_f(5,5) = yaw_rate_std_f^2;
R_f = R_f*3;
R_l = eye(5);
R_l(1,1) = x_std_l^2; R_l(2,2) = y_std_l^2; R_l(3,3) = yaw_std_l^2;
...
for epoch = 1:nepoch
    ...
    if use_lidar_f
        [pos_f_in_l, cov_f_in_l] = ominus_UT(pos_lidar_f, cov_lidar_f, 0.01);
        [pos_f_in_ref, cov_f_in_ref] = oplus_UT(pos_l_in_ref_com, pos_f_in_l, cov_l_in_ref_com, cov_f_in_l, 0.01);
        y_f_lidar = pos_f_in_ref;
        % Outlier Rejection
        C_f_lidar = eye(3);
        R_f_lidar = cov_f_in_ref*5;R_f_lidar(1,1) = R_f_lidar(1,1)*1;R_f_lidar(3,3) = R_f_lidar(3,3)*1;
    ...
    if use_lidar_f
        ...
        [X_f(1:3), P_f(1:3,1:3)] = KF_update(X_f(1:3), P_f(1:3,1:3), y_f_lidar, R_f_lidar, C_f_lidar);
    end  
    %----------------------------------------------------------------------
    % Save result
    % Follower
...
%--------------------------------------------------------------------------
% Plot result
figure('Name','Result of Follower','Color','w')
subplot(3,2,1)
...
subplot(3,2,2)
plot(result_f.timestamp,result_f.uselidar*20-10,'--','Color',[0 0 0 0.5])
hold on; grid on
...
figure('Name','Result of Leader','Color','w')
subplot(3,2,1)
